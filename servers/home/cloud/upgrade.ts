...
export async function main(ns: NS) {
	...
	const width = calcWidth(58) + 61.185
	...
	wl: while (true) {
		...
		if (tierCost <= budgetLeft) {
			for (const host of hosts) {
				...
				ns.print(host, " upgrade cost ", ns.format.number(ramUpgradeCost))
				lines++
		...
		for (const host of hosts) {
			...
			ns.print(host, " partial upgrade cost ", ns.format.number(ramUpgradeCost))
			lines++
