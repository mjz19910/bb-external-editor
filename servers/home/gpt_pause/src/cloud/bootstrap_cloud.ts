...
export async function main(ns: NS) {
	...
	while (true) {
		...
		if (liveMoney < fullTierCost) {
			ns.print(
				`not enough money for full tier, waiting... have ${
					ns.format.number(liveMoney)
				}, need ${ns.format.number(fullTierCost)}`,
			);
			lines++;
			await ns.sleep(5000);
			continue;
		...
		ns.print(
			`purchasing full tier of ${limit} servers at ${START_RAM} GB, total cost ${
				ns.format.number(fullTierCost)
			}`,
		);
		lines++;
