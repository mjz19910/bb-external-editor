...
export async function main(ns: NS) {
	...
	const runnerSrv = runnerInfo.server_info
	...
	if (targetSrv.moneyAvailable === void 0) return ns.tprint("missing moneyAvailable on targetSrv")
	if (targetSrv.moneyAvailable <= 0) return ns.tprint("no moneyAvailable on target server")
	const money_avail = targetSrv.moneyAvailable
	const hack_anal_threads = ns.hackAnalyzeThreads(target, money_avail)
	if (hack_anal_threads <= 0) return ns.tprint("hacking ", target, " is useless")
	...
	if (runnerThreads > runnerMaxThreads) return ns.tprint("not enough ram on runner server")
