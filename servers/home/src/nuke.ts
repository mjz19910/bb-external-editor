...
export async function main(ns: NS) {
	...
	if (srv.numOpenPortsRequired === void 0) return ns.tprint("required port count missing from cached server")
	if (srv.hasAdminRights) return ns.tprint("already have admin rights for " + target)
	if (srv.openPortCount >= srv.numOpenPortsRequired) {
		...
		ns.tprint("need " + srv.numOpenPortsRequired + " open ports to nuke " + target)
	}
}
